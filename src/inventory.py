...
def recommend_inventory(
    ...
    override_safety_pct: float | None = None,
) -> dict[str, Any]:
    """Recommended Stock = Forecast Demand + Safety Stock.

    Order Quantity = max(0, Recommended Stock − Current Inventory)

    By default the safety-stock percentage is derived automatically from
    demand variability in ``history``. Pass ``override_safety_pct`` (e.g. from
    a user-adjustable setting) to use a fixed percentage instead.
    """
    forecast_total = max(float(forecast_total), 0.0)
    current_inventory = max(float(current_inventory), 0.0)

    if override_safety_pct is not None:
        variability_pct = float(override_safety_pct)
    else:
